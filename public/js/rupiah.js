...
function numberToRupiah(number)
{
	...
	if (!positif)
	result = "(" + result + ")";
	//jika number = 0 atau kosong tanpa isi maka kembalikan juga kosongan tanpa format Rp.
	if(number == '' || number == 0)
	{
		return '';
	}else
...
function formatRupiahByElement(val)
{
	val.value = numberToRupiah(val.value);
}

function unformatRupiahByElement(val)
{
	val.value = rupiahToNumber(val.value);
}
